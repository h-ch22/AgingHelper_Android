package com.cj.aginghelper.authorization.ui
        ...
        Surface(color = AgingHelperColorPalette.current.background, modifier = Modifier.fillMaxSize()) {
            Column(verticalArrangement = Arrangement.Top, modifier = Modifier.padding(20.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Icon(imageVector = Icons.Rounded.LockPerson, contentDescription = null)
                    Text(text = "Authorization", color = AgingHelperColorPalette.current.txtColor, fontWeight = FontWeight.SemiBold)
                }
            }
        }
    }
